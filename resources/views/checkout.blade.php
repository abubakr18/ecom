        ...
        <div class="col-md-5 offset-md-1">
            ...
            <form action="{{ route('checkout.store') }}" method="POST">
                ...
                <div class="form-group">
                    <label for="email" class="light-text">عنوان البريد</label>
                    @guest
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="postal_code" class="light-text">رمز بريدي</label>
                            <input type="text" name="postal_code" class="form-control my-input" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label for="phone" class="light-text">الهاتف</label>
                        <input type="text" name="phone" class="form-control my-input" required>
                    </div>
                </div>
                <h2 style="margin-top:1em; margin-bottom:1em;">بيانات الدفع</h2>
                <div class="form-group">
                    <label for="name_on_card" class="light-text">الإسم على البطاقة</label>
                    <input type="text" name="name_on_card" class="form-control my-input" required>
                </div>
                <div class="form-group">
                    <label for="credit_card" class="light-text">بطاقة الإئتمان</label>
                    <input type="text" name="credit_card" class="form-control my-input" required>
                </div>
                <button type="submit" class="btn btn-success custom-border-success btn-block">أكمل الطلب</button>
            </form>
        </div>
        <div class="col-md-5 offset-md-1">
            <hr>
            ...
            @if (session()->has('coupon'))
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <form class="form-inline" action="{{ route('coupon.destroy') }}" method="POST" style="display:inline">
                            @csrf()
                            @method('DELETE')
                            <button class="inline-form-button" type="submit">إزالة</button>
                        </form>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <span class="light-text">المجموع الفرع الجديد</span>
                    </div>
                    <div class="col-md-4 offset-md-4">
            ...
            @if (!session()->has('coupon'))
                <form action="{{ route('coupon.store') }}" method="POST">
                    @csrf()
                    <label for="coupon_code">هل لديك قسيمة</label>
                    <input type="text" name="coupon_code" id="coupon" class="form-control my-input" placeholder="123456" required>
                    <button type="submit" class="btn btn-success custom-border-success btn-block">تطبيق القسيمة</button>
                </form>
            @endif
        </div>
    </div>
